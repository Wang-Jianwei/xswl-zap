...
(() => {
  ...
  assert(html.includes("edit-device-topology"));
  assert(html.includes("配置拓扑"));
  assert(!html.includes('data-target="sys-topo"'));
  assert(html.includes("btnTopologyBack"));
  assert(html.includes("topologyConfigPath"));
  assert(html.includes(" / topology config"));
  assert(html.includes("data-action=\"delete-vp\""));
  assert(html.includes("board-port-slot"));
  assert(html.includes("open-visual-topology"));
  assert(!html.includes("Launch Topology Editor"));
  assert(!html.includes("data-view=\"topology\""));
