...
- 仿真模式信号模型已增强：mock 驱动在扫频维度提供频率相关起伏，并在连续采集中加入轻量帧间动态，避免“平直无波动”观感。
- 频域渲染新增平滑曲线与噪声包络阴影；纵轴采用自适应范围，提升弱波动可见性。
...
- 修复 Canvas 预览脚本重复变量声明导致的 Webview `SyntaxError`（图像空白）问题，并增加回归断言防止再次引入同类错误。
...
- smoke 门禁脚本支持可选联动执行：`scripts/run_smoke_report_gate.ps1 -RunUiGrpcE2E`，用于在通过矩阵 smoke 后串行执行真实 gRPC UI E2E。
...
- 复制动作在图页内提供成功/失败状态反馈，减少上下文跳转。
- 复制过程先显示 `Copying...`，状态反馈 2 秒后自动淡出。
- 支持快捷键复制（`Ctrl/Cmd + C`，在非输入焦点场景）。
- 复制文本包含 `timestampNs`，并展示快捷键提示；无可复制数据时按钮给出原因提示。
- 状态反馈附带本地时间戳，支持 `Esc` 快速清除。
