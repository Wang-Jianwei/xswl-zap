...
- **虚拟VNA语义收敛**：Topology 定义的是“虚拟 VNA 设备构成拓扑”，Workspace 选择目标虚拟 VNA 执行测量。
- **后端互斥升级**：新增锁模型（owner / ttl / fencing token）与保存前 precheck（拓扑校验 + 资源冲突校验）。
- **双入口同编辑器**：System Studio 与 Workspace Quick Edit 复用同一拓扑编辑组件，不再分裂两套编辑能力。
- **冲突降级策略**：冲突时允许只读打开拓扑，禁止保存与拓扑改写，冲突解除后可恢复可写。
